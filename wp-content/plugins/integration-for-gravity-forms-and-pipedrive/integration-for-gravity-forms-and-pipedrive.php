<?php
/**
* Plugin Name: Integration for Gravity Forms and Pipedrive
* Description: Integrates Gravity Forms with Pipedrive allowing form submissions to be automatically sent to your Pipedrive account 
* Version: 1.2.0
* Requires at least: 3.8
* Author URI: https://www.crmperks.com
* Plugin URI: https://www.crmperks.com/plugins/gravity-forms-plugins/gravity-forms-pipedrive-plugin/
* Author: CRM Perks.
* Text Domain: gravity-forms-pipedrive-crm
...
class vxg_pipedrive {
 ...
 public  $url = 'https://www.crmperks.com';
  ...
  public  $id = 'vxg_pipedrive';
  public  $domain = 'vxg-pipedrive';
  public  $version = "1.2.0";
  public  $update_id = '30006';
  public  $min_gravityforms_version = '1.3.9';
  public $type = 'vxg_pipedrive_pro';
  public  $fields = null;
  public  $data = null;
  ...
  public static $title='Gravity Forms Pipedrive Plugin';  
  public static $path = ''; 
  public static $slug = '';
...
//  load_plugin_textdomain('gravity-forms-pipedrive-crm', FALSE,  $this->plugin_dir_name(). '/languages/' );
...
self::$plugin->instance();
  ...
  */
  ...
  if($type == "admin"){
     if($ver<4.2){
  ?>
    <div class="error vx_notice notice" data-id="<?php echo esc_html($id) ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Gravity Forms Pipedrive Plugin','gravity-forms-pipedrive-crm') ?>. </b><?php echo wp_kses_post($message);?> </span>
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php esc_html_e('Dismiss Notice','gravity-forms-pipedrive-crm') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  <div class="error vx_notice notice is-dismissible" data-id="<?php echo esc_html($id) ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Gravity Forms Pipedrive Plugin','gravity-forms-pipedrive-crm') ?>. </b> <?php echo wp_kses_post($message);?> </p>
  </div>    
  <?php
     }
  }else{
...
*/
  ...
  $message = sprintf(__("%sGravity Forms%s is required. %sPurchase it today!%s", 'gravity-forms-pipedrive-crm'), "<a href='http://www.gravityforms.com/'>", "</a>", "<a href='http://www.gravityforms.com/'>", "</a>");   
  }else if($installed === 2){ // not active
  $message = sprintf(__('Gravity Forms is installed but not active. %sActivate Gravity Forms%s to use the Gravity Forms Pipedrive Plugin','gravity-forms-pipedrive-crm'), '<strong><a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin=gravityforms/gravityforms.php'), 'activate-plugin_gravityforms/gravityforms.php').'">', '</a></strong>');  
  } else if($installed === 3){ // not supported
  $message = sprintf(__("A higher version of %sGravity Forms%s is required. %sPurchase it today!%s", 'gravity-forms-pipedrive-crm'), "<a href='http://www.gravityforms.com/'>", "</a>", "<a href='http://www.gravityforms.com/'>", "</a>");
  }  
  ...
  */
  ...
  if(current_user_can('administrator') && !empty($_REQUEST['VXGPipedriveError'])) { /// 
  $confirmation .= sprintf(__('%sThe entry was not added to Pipedrive because: %s. %sYou are only being shown this because you are an administrator. Other users will not see this message.%s', 'gravity-forms-pipedrive-crm'), '<div class="error" style="text-align:center; color:#790000; font-size:14px; line-height:1.5em; margin-bottom:16px;background-color:#FFDFDF; margin-bottom:6px!important; padding:6px 6px 4px 6px!important; border:1px dotted #C89797">','<strong>'.esc_html($_REQUEST['VXGPipedriveError']). '</strong>', '<br /><em>', '</em></div>');
  }
  return $confirmation;
  }
...
//  $value=implode(", ",$value);
  ...
  */
  ...
  printf('<input type="checkbox" name="'.$this->id.'_update" id="'.$this->id.'_update" value="1" /><label for="'.$this->id.'_update" title="%s">%s</label><br /><br />', esc_html__('Create or update this entry in Pipedrive. The fields will be mapped according to the form feed settings.', 'gravity-forms-pipedrive-crm'), esc_html__('Send to Pipedrive', 'gravity-forms-pipedrive-crm'));
  } else {
...
if(!current_user_can($this->id."_send_to_crm")){return; }
  ...
  //logs button
    ...
    $button.= '<a class="button button-large button-secondary alignright" style="margin-left:5px; margin-top:5px; " title="'.__('Go to Pipedrive Logs','gravity-forms-pipedrive-crm').'" href="'.$log_url.'">'.__('Pipedrive Logs','gravity-forms-pipedrive-crm').'</a>';
  ...
  */
  ...
  RGFormsModel::add_note($id, 0, esc_html__('Gravity Forms Pipedrive Plugin','gravity-forms-pipedrive-crm'), $note);
  }
...
do_action('plugin_status_'.$this->type,'activate');  
  ...
  */
  ...
  $crm_tooltips = array(
    'vx_feed_name' => '<h6>' . esc_html__('Feed Name', 'gravity-forms-pipedrive-crm') . '</h6>' . esc_html__('Enter feed name of your choice.', 'gravity-forms-pipedrive-crm'),
  'vx_sel_object' => '<h6>' .__('Pipedrive Object', 'gravity-forms-pipedrive-crm') . '</h6>' . esc_html__('Select the Object to Create when a Form is Submitted.', 'gravity-forms-pipedrive-crm'),
   'vx_sel_account' => '<h6>' .__('Pipedrive Account', 'gravity-forms-pipedrive-crm') . '</h6>' . esc_html__('Select the Pipedrive account you would like to export entries to.', 'gravity-forms-pipedrive-crm'),
  'vx_sel_form' => '<h6>' . esc_html__('Gravity Form', 'gravity-forms-pipedrive-crm') . '</h6>' . esc_html__('Select the Gravity Form you would like to integrate with Pipedrive. Contacts generated by this form will be automatically added to your Pipedrive account.', 'gravity-forms-pipedrive-crm'),
  
  'vx_map_fields' => '<h6>' . esc_html__('Map Standard Fields', 'gravity-forms-pipedrive-crm') . '</h6>' . esc_html__('Associate your Pipedrive fields to the appropriate Gravity Form fields.', 'gravity-forms-pipedrive-crm'),
  
  'vx_optin_condition' => '<h6>' . esc_html__('Opt-In Condition', 'gravity-forms-pipedrive-crm') . '</h6>' . esc_html__('When the opt-in condition is enabled, form submissions will only be exported to Pipedrive when the condition is met. When disabled all form submissions will be exported.', 'gravity-forms-pipedrive-crm'),
  
  'vx_manual_export' => '<h6>' . esc_html__('Manual Export', 'gravity-forms-pipedrive-crm') . '</h6>' . esc_html__('If you do not want all entries sent to Pipedrive, but only specific, approved entries, check this box. To manually send an entry to Pipedrive, go to Entries, choose the entry you would like to send to Pipedrive, and then click the "Send to Pipedrive" button.', 'gravity-forms-pipedrive-crm'),
  
    'vx_entry_notes' => '<h6>' . esc_html__('Entry Notes', 'gravity-forms-pipedrive-crm') . '</h6>' . esc_html__('Enable this option if you want to synchronize Gravity Forms entry notes to Pipedrive Object notes. For example , when you add a note to a Gravity Forms entry, it will be added to the Pipedrive Object selected in the feed.', 'gravity-forms-pipedrive-crm'),
    
      'vx_primary_key' => '<h6>' . esc_html__('Primary Key', 'gravity-forms-pipedrive-crm') . '</h6>' . esc_html__('Which field should be used to update existing objects?', 'gravity-forms-pipedrive-crm'),
    

    'vx_source_check'=>'<h6>' . esc_html__('Enable Lead Source', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Enable this option if you want to assign a lead source to this object.','gravity-forms-pipedrive-crm'),
        
'vx_sources'=>'<h6>' . esc_html__('Pipedrive Lead Sources', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Get Lead Sources list from pipedrive.','gravity-forms-pipedrive-crm'),
'vx_source'=>'<h6>' . esc_html__('Select Lead Source', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Which Lead Source should be assigned to this object.','gravity-forms-pipedrive-crm'),

    'vx_status_check'=>'<h6>' . esc_html__('Enable Ticket Status', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Enable this option if you want to assign a lead status to this object.','gravity-forms-pipedrive-crm'),    
'vx_status_list'=>'<h6>' . esc_html__('Pipedrive Ticket Status List', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Get Ticket Status list from pipedrive.','gravity-forms-pipedrive-crm'),
'vx_status'=>'<h6>' . esc_html__('Select Ticket Status', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Which Ticket Status should be assigned to this object.','gravity-forms-pipedrive-crm'),
'vx_priority'=>'<h6>' . esc_html__('Select Ticket Priority', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Which Ticket Priority should be assigned to this object.','gravity-forms-pipedrive-crm'),



   'vx_assign_person'=>'<h6>' . esc_html__('Pipedrive Person', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Enable this option if you want to assign a person to this object.','gravity-forms-pipedrive-crm'),
   
      'vx_assign_check'=>'<h6>' . esc_html__('Assign Person', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Enable this option if you want to assign a user to this conversation.','gravity-forms-pipedrive-crm'),
   'vx_sel_person'=>'<h6>' . esc_html__('Select Person', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Person created by selected feed will be added to this feed Object.','gravity-forms-pipedrive-crm'),
   
   
   'vx_owner_check'=>'<h6>' . esc_html__('Assign Owner', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Enable this option if you want to assign another object owner.','gravity-forms-pipedrive-crm'),
   
   'vx_owners'=>'<h6>' . esc_html__('Pipedrive Users', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Get Users list from Pipedrive','gravity-forms-pipedrive-crm'),
   
      'vx_sel_owner'=>'<h6>' . esc_html__('Select Owner', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Select a user as a owner of this object','gravity-forms-pipedrive-crm'),
   
      'vx_box_check'=>'<h6>' . esc_html__('Add to mailbox', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Enable this option if you want to add conversation in a mailbox.','gravity-forms-pipedrive-crm'),
   
   'vx_boxes'=>'<h6>' . esc_html__('Pipedrive Mailboxes', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Get mailboxes from Pipedrive','gravity-forms-pipedrive-crm'),
   
      'vx_sel_box'=>'<h6>' . esc_html__('Select Mailbox', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Select a mailbox in which you want to add conversation.','gravity-forms-pipedrive-crm'),
   
   
   
   'vx_order_notes'=>'<h6>' . esc_html__('Entry Notes', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Enable this option if you want to synchronize Entry notes to Pipedrive Object notes. For example, when you add a note to a Entry, it will be added to the Pipedrive Object selected in the feed.','gravity-forms-pipedrive-crm'),
  

         'vx_entry_note'=>'<h6>' . esc_html__('Entry Note', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Check this option if you want to send more data as CRM entry note.', 'gravity-forms-pipedrive-crm'),
   'vx_note_fields'=>'<h6>' . esc_html__('Note Fields', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Select fields which you want to send as a note', 'gravity-forms-pipedrive-crm'),
   'vx_disable_note'=>'<h6>' . esc_html__('Note Fields', 'gravity-forms-pipedrive-crm') . '</h6>' .__('Enable this option if you want to add note only for new CRM entry', 'gravity-forms-pipedrive-crm')
...
/**
  ...
  */
  ...
  $status=0;
  if(class_exists('vxg_pipedrive_wp')){
      $status=1;
  }else if( file_exists(WP_PLUGIN_DIR.'/gravity-forms-pipedrive-crm/gravity-forms-pipedrive-crm.php')) {
  $status=2;
  } 
  return $status;
  }
...
public function do_actions(){
  ...
  */
  ...
  $this->api=$api= new vxg_pipedrive_api($crm);
  ...
  */
  ...
  $this->data=new vxg_pipedrive_data();
  return $this->data;
  }
...
$value=$this->process_tags($entry,$form,$value);   
  ...
  if(isset($data['owner']) && $data['owner'] == "1" && !empty($data['user'])){
   $temp['owner_id']=array('value'=>$data['user'],'label'=>'User id');  
  }
...
//  var_dump($feed_arr); echo '<hr>'; var_dump($data);  die();
  ...
  if($this->post('id',$res)!=""){ 
      ...
      if($action == "Added"){
          if(empty($res['link'])){
  $msg=sprintf(__('Successfully Added to Pipedrive (%s) with ID # %s .', 'gravity-forms-pipedrive-crm'),$feed['object'],$res['id']);
          }else{
  $msg=sprintf(__('Successfully Added to Pipedrive (%s) with ID # %s . View entry at %s', 'gravity-forms-pipedrive-crm'),$feed['object'],$res['id'],$res['link']);
          }
  $screen_msg=__( 'Entry added in Pipedrive', 'gravity-forms-pipedrive-crm');
      }else{
            if(empty($res['link'])){
  $msg=sprintf(__('Successfully Updated to Pipedrive (%s) with ID # %s . View entry at %s', 'gravity-forms-pipedrive-crm'),$feed['object'],$res['id'],$res['link']);   
            }else{
  $msg=sprintf(__('Successfully Updated to Pipedrive (%s) with ID # %s .', 'gravity-forms-pipedrive-crm'),$feed['object'],$res['id']);   
            }
     $screen_msg=__( 'Entry updated in Pipedrive', 'gravity-forms-pipedrive-crm');
      }
   
  
  }else if($this->post('status',$res) == 4){
  $screen_msg=$msg=__( 'Entry filtered', 'gravity-forms-pipedrive-crm');    
  }else if($this->post('status',$res) == 6){ 
      $status='2';   $screen_msg=$error=$res['error'];
  }else{
  $status=0; $screen_msg_class="error";
  $screen_msg=__('Errors when adding to Pipedrive. Entry not sent! Check the Entry Notes below for more details.' , 'gravity-forms-pipedrive-crm' );
  if($log_id!=""){
      //message for  bulk actions in logs
  $screen_msg=__('Errors when adding to Pipedrive. Entry not sent' , 'gravity-forms-pipedrive-crm' );    
  }
  $msg=sprintf(__('Error while creating %s', 'gravity-forms-pipedrive-crm'),$feed['object']);
...
//  } 
  ...
  $log_link=' <a href="'.$log_url.'" class="vx_log_link" data-id="'.$insert_id.'">'.__('View Detail','gravity-forms-pipedrive-crm')."</a>";
 $screen_msg.=$log_link;
    }
    if($post_comment){
...
public function process_tags($entry,$form,$value,$crm_field_id='',$custom=''){
  ...
  */
  ...
  $detail["Form Id"]=$form['id'];
    ...
    $email_info=array("msg"=>$info['msg'],"title"=>__('Pipedrive','gravity-forms-pipedrive-crm')." Error","info_title"=>"More Detail","info"=>$detail);
...
$vxg_pipedrive=new vxg_pipedrive();
$vxg_pipedrive->instance();
$vx_gf['vxg_pipedrive']='vxg_pipedrive';
